function pi_autodet_batch_ST

% Pull in all xwavs of a folder and subfolder to run matched filter detector
% for explosions.
global REMORA

settings = REMORA.pi.settings;
nSamples = REMORA.pi.settings.nSamples;
c2_offset = REMORA.pi.settings.c2_offset;
...
SearchPathMask = REMORA.pi.settings.baseDir;
DetDir = REMORA.pi.settings.outDir; % Modified to save files to different folder than the input folder.
SearchRecursiv = REMORA.pi.settings.recursSearch; % Setting to 1 searches through all subfolders in the selected folder, setting to 0 only searches the selected folder.
...
if ~isdir(REMORA.pi.settings.outDir)
    mkdir(REMORA.pi.settings.outDir)
end
...
for fidx = 1:size(FileList,1)
    ...
    for idx = 1:inc
        ...
        if rStart>=fileSampleNum ||rStop>fileSampleNum
            warning('File has fewer than expected samples. Continuing to next file.');
            continue
        end
