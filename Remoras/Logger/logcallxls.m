    ...
    for hidx = 1:length(detection.Headers)
        ...
        if ismac
            switch lower(detection.Headers{hidx})
                ...
                case 'starttime'
                    detection.Sheet.StartTime(currentRow) = entry.pickstartdisplay - date_epoch('excel')+(365*3+366+1); %datetime(entry.pickstartdisplay - date_epoch('excel'),'ConvertFrom','datenum');
                case 'endtime'
                    if isfield(entry, 'pickenddisplay')
                        detection.Sheet.EndTime(currentRow) = entry.pickenddisplay - date_epoch('excel')+(365*3+366+1); %datetime(entry.pickenddisplay - date_epoch('excel'),'ConvertFrom','datenum');
                    end
                case 'eventnumber'
