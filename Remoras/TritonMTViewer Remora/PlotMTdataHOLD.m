function PlotMTdataHOLD(handles)
...
sp = 'Bryde''s Whale' ;
global all sync
if (get(sync.handles.pushbutton2,'Value')||get(sync.handles.checkbox1,'Value'))
    ...
    axes(sync.handles.axes6)
        ...
        titl = sprintf('%s %s %s %s',sp, tt, datestr(sync.handles.strt)) ;
        title(titl) ;
        set(gca,'XTick',[],'FontSize',9,'Tag','Depth','NextPlot','add')
        xlabel('')
    axes(sync.handles.axes7)
        cla
