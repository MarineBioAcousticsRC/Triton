...
%%
    ...
    hs2 = subplot(1,3,2);
    imagesc(1:size(Tfinal{1},1),p.barInt(1:s.maxICIidx),...
        [Tfinal{1,2}./max(Tfinal{1,2},[],2)]')
    set(gca,'ydir','normal')
...
%     xlim([min(p.barRate),max(p.barRate)])
    ...
    imagesc(1:size(Tfinal{1},1),1:p.maxDur,[Tfinal{1,10}./max(Tfinal{1,10},[],2)]')
    set(gca,'ydir','normal')
    xlabel('Bin Number','FontSize',12)
    ...
    fPlot = f;
    if length(fPlot)~= size(Tfinal{1,1},2) &&... 
        length(f(s.stIdx:s.edIdx))~=size(Tfinal{1,1},2)
...
%     end
    ...
    imagesc(1:size(Tfinal{1},1),fPlot,Tfinal{1,1}') ;
    ...
    colormap(jet)
    drawnow
    mtit(400,[siteNameNo_,': Cluster ',num2str(iType)],'FontSize',12);
...
%%
    if s.saveOutput
        figName = fullfile(outDir,sprintf('%s_type%d',s.outputName,iType));
        print(gcf,'-dtiff','-r600',[figName,'.tif'])
        saveas(gcf,[figName,'.fig'])
        fprintf('Done saving cluster %0.0f\n',iType)
    end
