...
% PARAMS.ltsa.fod = fopen(fullfile(PARAMS.ltsa.outdir,PARAMS.ltsa.outfile),'r+');
nch = 1; % PARAMS.ltsa.nch(1); hard coded to single channel
% if PARAMS.ltsa.ch > 1 % if I want to do more than one channel
%     PARAMS.ltsa.fods = zeros(nch, 1);
% else
    PARAMS.ltsa.fods = zeros(1,1);
% end
curr_ofile = PARAMS.ltsa.outfile;
PARAMS.ltsa.outfiles = [];

for ch = 1:nch
    % I don't actually want to do both channels so possibly ADD
    % a check here that ch matches PARAMS.ltsa.ch then run
    if nch ~= 1
