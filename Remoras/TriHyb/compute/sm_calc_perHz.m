...
for i = 1:length(allDays)
    ...
    surf(ax1, time_matrix, F, psd_tf, 'Linestyle', 'none');
    ...
    pctVals = prctile(psd_tf, percentiles, 2);  % size: [nFreqBands x nPercentiles]
    ...
    for n = 1:length(percentiles)
        semilogy(ax2, pctVals(:, n), F, 'LineWidth', 1.2);
    end
    ...
    freqDimID = netcdf.defDim(ncid, 'frequency', length(F));
    ...
    netcdf.putVar(ncid, freqVarID, double(F));
    netcdf.putVar(ncid, psdVarID, double(psd_tf));
