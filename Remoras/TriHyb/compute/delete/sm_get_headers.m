...
for k = 1:PARAMS.ltsa.nxwav            % loop over all files in directory
    ...
    if PARAMS.ltsa.ftype == 1       % do the following for wav files
        ...
        %         mm = [];
        %         [mm dd] = wavfinfo(fullfile(PARAMS.ltsa.inputDir,PARAMS.ltsa.fname(k,:)));
        %         if isempty(mm)
        %             disp(dd)
        %             disp(fullfile(PARAMS.ltsa.inputDir,PARAMS.ltsa.fname(k,:)))
        %             disp('Run wavDirTestFix.m on this directory first, then try again')
        %             return
        %         end

        % read wav header and fill up PARAMS
        %         [y, fs, PARAMS.ltsa.nBits, OPTS] = wavread( fullfile(PARAMS.ltsa.inputDir,PARAMS.ltsa.fname(k,:)),10);
        ...
        if PARAMS.ltsa.recursiveSearch == 1
            try
                info = audioinfo(fullfile(PARAMS.ltsa.inputDir, '\*\', [PARAMS.ltsa.FilenamePattern_ '.x.wav']));    % xwav files
            catch ME
                disp(ME.message)
                dmsg = sprintf('Is %s a real wave file?', ...
                    fullfile(PARAMS.ltsa.inputDir,PARAMS.ltsa.fname(k,:)));
                disp(dmsg);
        ...
        elseif PARAMS.ltsa.recursiveSearch == 0
            try
                info = audioinfo(fullfile(PARAMS.ltsa.inputDir,PARAMS.ltsa.fname(k,:)));
            catch ME
                disp(ME.message)
                dmsg = sprintf('Is %s a real wave file?', ...
                    fullfile(PARAMS.ltsa.inputDir,PARAMS.ltsa.fname(k,:)));
        ...
        % timing stuff:
        dnums = sm_wavname2dnum(PARAMS.ltsa.fname(k,:));
        if isempty(dnums)
    ...
    elseif PARAMS.ltsa.ftype == 2               % do the following for xwavs
        ...
        fid = fopen(fullfile(PARAMS.ltsa.inputDir,PARAMS.ltsa.fname(k,:)),'r');
