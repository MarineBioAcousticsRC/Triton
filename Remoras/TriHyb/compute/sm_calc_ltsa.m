...
h = loadbar(['Creating HBD Products ',num2str(lIdx),'/',num2str(size(PARAMS.ltsa.ltsaIdx,1))]);
pcntDone = 0;
    ...
    for r = 1:nrf                   % loop over each raw file in xwav
        ...
        if m <=PARAMS.ltsa.endIdx
            ...
            for n = 1 : PARAMS.ltsa.nave(m) % loop over the number of spectral averages
                ...
                % calculate spectra
                [ltsa,freq] = pwelch(data,window,50,PARAMS.ltsa.nfft,PARAMS.ltsa.fs);   % pwelch is supported psd'er
