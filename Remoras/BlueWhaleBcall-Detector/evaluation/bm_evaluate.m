function varargout = bm_evaluate(varargin)
...
% Last Modified by GUIDE v2.5 02-Apr-2020 07:16:29

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name', mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @bm_evaluate_OpeningFcn, ...
    'gui_OutputFcn',  @bm_evaluate_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
function bm_evaluate_OpeningFcn(hObject, ~, handles, varargin)
...
function varargout = bm_evaluate_OutputFcn(~, ~, handles)
...
if ~isfield(handles,'ltsaData') || isempty(handles.ltsaData)
    ...
    bm_draw_ltsa(handles);
    
    if ~handles.eval
        times = handles.times;
    else
        times = handles.timesEval;
    end
    
    % update percentage processed
    perc = round(handles.ViewEnd/size(times,1)*100);
    % only show 100 when it is really 100, keep 99 if round percentage is 100
    if perc == 100 && (handles.ViewEnd/size(times,1)*100) ~=100
       perc = 99; 
    end
...
while(size(handles.ltsaData,2) < floor(handles.PlotLengthVal*60*60/handles.ltsa.tave))...
        && (handles.j <= size(times,1))
    
    % Read the ltsa data portion of the file
    ltsaData = bm_read_ltsadata(handles,times(handles.j,1),times(handles.j,2));
...
if(handles.ViewEnd>=size(times,1))
    set(handles.motion_forwards,'Enable','off')
...
while(size(handles.ltsaData,2) < floor(handles.PlotLengthVal*60*60/handles.ltsa.tave))...
    ...
    % Read the ltsa data portion of the file
    ltsaData = bm_read_ltsadata(handles,times(viewStart,1),times(viewStart,2));
...
% update text
set(handles.percent_completed,'String',round(handles.ViewEnd/size(times,1)*100))
handles.marker_count = handles.marker_count+length(handles.markers);
...
function all_Bm_Callback(hObject, eventdata, handles)
...
if ~handles.eval
    ...
    handles.BmLabels(handles.ViewStart:handles.ViewEnd) = {'Bm'};
    BmLabels = handles.BmLabels;
else
    times = handles.times(handles.idxRandSamples,:);
    handles.BmLabelsEval(handles.ViewStart:handles.ViewEnd) = {'Bm'};
    BmLabels = handles.BmLabelsEval;
end

if ~handles.eval
    save(fullfile(handles.DetectionFilePath,handles.DetectionFile), 'BmLabels','-append')
    ...
    sh_write_labels(fullfile(handles.DetectionFilePath,handles.LabelFile), ...
        times - datenum([2000 0 0 0 0 0]), BmLabels, 'Binary', true);
    
    % rewrite csv file
    if handles.settingsRemora.saveCsv
        handles.CsvFile = ['Ship_detections_',filename{1},'.csv'];
        sh_write_csv_file(fullfile(handles.DetectionFilePath,handles.CsvFile),times,BmLabels)
    end
else
    bm_write_eval_csv_file(handles,0);
end
...
function all_no_Bm_Callback(hObject, eventdata, handles)
...
if ~handles.eval
    times = handles.times;
    handles.BmLabels(handles.ViewStart:handles.ViewEnd) = {'ambient'};
    BmLabels = handles.BmLabels;
else
    times = handles.timesEval;
    handles.BmLabelsEval(handles.ViewStart:handles.ViewEnd) = {'ambient'};
    BmLabels = handles.BmLabelsEval;
end
...
function subset_Bm_Callback(hObject, eventdata, handles)
...
if ~handles.eval
    times = handles.times;
    handles.BmLabels(handles.ViewStart+selected-1) = {'Bm'};
    BmLabels = handles.BmLabels;
else
    times = handles.timesEval;
    handles.BmLabelsEval(handles.ViewStart+selected-1) = {'Bm'};
    BmLabels = handles.BmLabelsEval;
end
...
if ~handles.eval
    ...
    filename = split(handles.LtsaFile,'.ltsa');
    % rewrite tlab file
    handles.LabelFile = ['Bm_labels_',filename{1},'.tlab'];
    sh_write_labels(fullfile(handles.DetectionFilePath,handles.LabelFile), ...
        times - datenum([2000 0 0 0 0 0]), BmLabels, 'Binary', true);
    % rewrite csv file
    if handles.settingsRemora.saveCsv
...
function subset_no_Bm_Callback(hObject, eventdata, handles)
...
if ~handles.eval
    times = handles.times;
    handles.BmLabels(handles.ViewStart+selected-1) = {'ambient'};
    BmLabels = handles.BmLabels;
else
    times = handles.timesEval;
    handles.BmLabelsEval(handles.ViewStart+selected-1) = {'ambient'};
    BmLabels = handles.BmLabelsEval;
end
...
if ~handles.eval
    ...
    % rewrite csv file
    if handles.settingsRemora.saveCsv
        handles.CsvFile = ['Bm_detections_',filename{1},'.csv'];
        sh_write_csv_file(fullfile(handles.DetectionFilePath,handles.CsvFile),times,BmLabels)
    end
else
    bm_write_eval_csv_file(handles,0);
...
if (get(hObject,'Value') == get(hObject,'Max'))
    ...
    if strcmp(enabledEval,'off')
        ...
        set(handles.totalNum,'String',size(handles.times,1))
    end
else
	enabledEval = get(handles.nSamples,'Enable');
...
handles.replot = 1;
handles.timesEval = [];
handles.BmLabelsEval = [];
handles.evalTable = [];

% create file with user evaluation
handles.timesEval = handles.times(handles.idxRandSamples,:);
handles.BmLabelsEval = handles.BmLabels(handles.idxRandSamples);
% plot_ltsa_Callback(hObject, eventdata, handles)
handles = motion_forwards_Callback(hObject, eventdata, handles);

bm_write_eval_csv_file(handles,1);
