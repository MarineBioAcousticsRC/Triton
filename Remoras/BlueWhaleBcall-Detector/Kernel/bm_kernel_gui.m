...
% Kernel site Text
labelStr = 'Site of kernel calls';
btnPos=[x(1)+xbuff  y(4) w dy];
REMORA.bm.kernel_verify.kernelSiteTxt = uicontrol(REMORA.fig.bm.kernel,...
    'Style','text',...
    'Units','normalized',...
        'HorizontalAlignment','left',...
    'Position',btnPos,...
...
% Kernel Site Editable Text
labelStr=num2str(REMORA.bm.settings.kernelSite);
btnPos=[x(2)-xbuff y(4) w dy];
REMORA.bm.kernel_verify.kernelSiteEdTxt = uicontrol(REMORA.fig.bm.kernel,...
    'Style','edit',...
    'Units','normalized',...
    'Position',btnPos,...
    'BackgroundColor',bgColor,...
    ...
    'FontUnits','normalized', ...
    'Visible','on',...
    'Callback','bm_kernel_control(''setKernelSite'')');
