function visWeeklyBinned(queryEng, varargin)


%for BINNED detections
%This code basically grabs all binned granularity start times.
% Works for HOURLY bins only:
%  -it grabs all of the Detection start times
%  -sets them back to the top of the hour
%  -sets the end date as an hour from then
%  -if there are duplicate hours, they get removed
%  -then it plots, just like the encounter plots
            ...
            for didx=1:depl_count
                ...
                if iseff
                    ...
                    if isempty(queries(quer_count).Detections)
                        ...
                        starts_vec(:,5:6) = 0;
                        plots(plotIdx).Bins = datenum(starts_vec);
                        %loop in the end hour
                        for ridx = 1:length(plots(plotIdx).Bins)
                            plots(plotIdx).Bins(ridx,2) = addtodate(...
                                plots(plotIdx).Bins(ridx,1),60,'minute');
                        end
...
for i=1:length(plots)
    ...
    %%%LOOP THRU EFFORTS, TRUNCATE START/END IF LENGTHLESS BREAKS%%%
