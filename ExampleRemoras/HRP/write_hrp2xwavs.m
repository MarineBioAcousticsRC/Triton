function write_hrp2xwavs(infilename,hdrfilename,outdir,d,opts)
% opts (optional struct) controls compression to x.flac:
%   opts.compress   write x.flac instead of x.wav  (default false)
%   opts.keepXwav   also keep the x.wav            (default false)
%   opts.flac       path to the flac tool          (found automatically)
%
% Compression is off by default. It is lossless -- the x.wav can be
% regenerated byte for byte -- and typically halves the archive, but this is
% the function that produces the lab's primary data, so turning it on is a
% deliberate choice rather than something that happens by upgrading.
%
% With compression on, each x.wav is written as now, converted, verified and
% then deleted. It is written first rather than compressed on the fly because
% flac needs a complete RIFF file to preserve the header from, and because a
% conversion that cannot be verified must leave the x.wav untouched.
%
...
if ~isfield(opts,'keepXwav'); opts.keepXwav = false; end
if ~isfield(opts,'flac');     opts.flac     = '';    end
if opts.compress
    % Check for the tool before writing anything. Discovering it is missing
    % after a few hours of decompressing raw HARP data would be a poor time to
    % find out, and the whole run would have to be repeated.
    [flacTool, whyNot] = flac_exe(opts.flac);
    if isempty(flacTool)
        disp(['Error - compression requested but ',whyNot])
        return
    end
    opts.flac = flacTool;
    disp(['Writing x.flac using ',flacTool])
    if opts.keepXwav
        disp('  keeping the x.wav files as well')
    end
end
nConverted = 0; savedIn = 0; savedOut = 0;
...
%     for k = 70:70
    ...
    if opts.compress
        % Verified conversion: the x.wav is removed only once the audio has
        % been decoded back and compared, and the preserved harp header has
        % been checked byte for byte against the file being deleted.
        before = dir(fname);
        [cok, cout, cmsg] = xwav2flac(fname, 'keepXwav', opts.keepXwav, ...
            'flac', opts.flac);
        if ~cok
            disp(['Error - could not compress ',name,': ',cmsg])
            disp('  the x.wav has been kept; stopping so the rest is not lost')
            bflag = 1;
            break
        end
        after = dir(cout);
        nConverted = nConverted + 1;
        savedIn = savedIn + before.bytes; savedOut = savedOut + after.bytes;
        if dflag
            disp(['Compressed to ',cout,' (',num2str(after.bytes),' bytes)'])
        end
    end
end
% close raw HRP file
...
if opts.compress && nConverted > 0
    disp(['Compressed ',num2str(nConverted),' files, ', ...
        num2str(savedIn/1e6,'%.0f'),' MB -> ',num2str(savedOut/1e6,'%.0f'), ...
        ' MB (',num2str(100*(1-savedOut/savedIn),'%.1f'),'% saved)'])
end
