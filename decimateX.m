function odata = decimateX(idata,r,nfilt,option)
...
%
% modified decimate.m to push corner frequency up with Chebyshev filter
% 221005 smw
...
if fopt == 0	% FIR filter
    ...
    rip = .05;	% passband ripple in dB
     fc = .8/r;  % original corner frequency
%    fc = 0.84/r;   % try corner nearer to Nyquist
    [b,a] = cheby1(nfilt, rip, fc);  % 'original' filter call
    while all(b==0) || (abs(filtmag_db(b,a,fc)+rip)>1e-6)
        nfilt = nfilt - 1;
        if nfilt == 0
            break
        end
        [b,a] = cheby1(nfilt, rip, fc);   % original
    end
    if nfilt == 0
